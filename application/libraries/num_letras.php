...
function NumtoletrasPesos($xcifra,$Tipo){
			...
			switch ($xy) 
				...
				case 1: // checa las centenas
					if (substr($xaux, 0, 3) < 100) // si el grupo de tres dígitos es menor a una centena ( < 99) no hace nada y pasa a revisar las decenas
						...
						if ($xseek)
							{
							$xsub = subfijo($xaux,$Tipo); // devuelve el subfijo correspondiente (Millón, Millones, Mil o nada)
							if (substr($xaux, 0, 3) == 100)
								$xcadena = " ".$xcadena." ".$VALOR1." ".$xsub;
							else
				...
				case 2: // checa las decenas (con la misma lógica que las centenas)
					if (substr($xaux, 1, 2) < 10)
						...
						if ($xseek)
							{
							$xsub = subfijo($xaux,$Tipo);
							if (substr($xaux, 1, 2) == 20)
								$xcadena = " ".$xcadena." ".$VALOR2." ".$xsub;
							else
								$xcadena = " ".$xcadena." ".$xseek." ".$xsub;
							$xy = 3;
							}
			...
			switch ($xz)
				...
				case 0:
					if (trim(substr($XAUX, $xz * 6, 6)) == "1")
						$xcadena.= $VALOR5." BILLON ";
					else
						$xcadena.= " BILLONES ";
					break;
				case 1:
					if (trim(substr($XAUX, $xz * 6, 6)) == "1")
						$xcadena.= $VALOR5." MILLON ";
					else
						$xcadena.= " MILLONES ";
					break;
				case 2:
					if ($xcifra < 1 )
						{
						$xcadena = $VALOR6." $xdecimales/100 ".$MONEDAS;
						}
					if ($xcifra >= 1 && $xcifra < 2)
						{
						$xcadena = $VALOR7." $xdecimales/100 ".$MONEDAS;
						}
					if ($xcifra >= 2)
						{
						$xcadena.= " ".$VALOR8." $xdecimales/100 ".$MONEDAS; // 
						}
					break;
				} // endswitch ($xz)
			} // ENDIF (trim($xaux) != "")
...
function subfijo($xx,$Tipo){ // esta función regresa un subfijo para la cifra
